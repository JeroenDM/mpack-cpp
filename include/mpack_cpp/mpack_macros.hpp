...
#define MPACK_CPP_DEFINE(Type, ...) \
    ...
    void from_message_pack(mpack_node_t& node) { \
        BOOST_PP_SEQ_FOR_EACH(MPACK_READ_FIELD_OP, node, BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__)) \
    }
...
#define MPACK_CPP_EXPECT_DEFINE(Type, ...) \
    ...
    void from_message_pack(mpack_reader_t& reader) { \
        BOOST_PP_SEQ_FOR_EACH(MPACK_EXPECT_READ_FIELD_OP, reader, BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__)) \
    }
